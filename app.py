...
def generate_response(uploaded_file, employees_info, project_name, num_employees):
    if uploaded_file is not None:
        question1 = f"""Представь, что геймдев разработчик и у тебя есть игра, где сюжет разворачивается вокруг проекта. Твоя задача создать для этого проекта {num_employees} сотрудника(-ов) с указанными именами и данными из ВУЗа и 
        ...
        1.3. Написать какие обязанности будет исполнять.
         Для других сотрудников структура и подход аналогичный. """
        ...
        а сам только хочешь открыть компанию и заниматься этим делом. Ответ дай в следующем формате:
        ...
        4. Ожидаемые результаты через год, в случае если инвестор выделит тебе миллион рублей на проект
        5. Кто будет целевой аудиторией для проекта
        Дай ответ на русском. Пиши ответ не по пунктам, а сплошным текстом. Обязательное условие - твой ответ должен не превышать по объёму 1000 символов без учета пробелов."""
        ...
        - выявить основные проблемы, возникающие у потребителя, которые решает продукт;
        - предположить какие характеристики продукта будут ключевыми для конечного потребителя и обосновать выбор.
        ...
        Свой ответ пиши в формате отчёта. Если информации из файла недостаточно, то ответь на вопросы, используя свой опыт в этой сфере. Не пиши, что ты брал информацию в файле"""
